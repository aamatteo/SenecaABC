...
"$Id: GUIStick.cxx,v 1.1 2017/01/09 16:37:33 rvanpaassen Exp $";
...
GUIStick::GUIStick(Entity* e, const char* part, const
  ...
  fci_token(getId(), NameSet(getEntity(), "FlightControlInput", "trim")),
...
bool GUIStick::isPrepared()
{
  ...
  I_MOD(classname << " has " << (fci_token.isValid() ? "valid" : "invalid") 
	<< " FlightControlInput trim token");
...
{
  // CS CitationModel hack
  if(fci_token.isValid()) {
    if(fci_token.getNumWaitingEvents(ts)) {
      try {
	EventReader<FlightControlInput> fci(fci_token, ts);
	
	if(use_sc) {
	  gtk_range_set_value(GTK_RANGE(sc_thrl), 
			      (fci.data().pla1-0.2356)/0.8465 );
	  gtk_range_set_value(GTK_RANGE(sc_thrr),
			      (fci.data().pla1-0.2356)/0.8465 );
	}
      } catch(Exception& e) {
